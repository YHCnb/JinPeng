  ...
  <view class="t-cropper" v-show="props.imageUrl" disable-scroll>
    ...
    <view class="t-preview-container" :class="{ showPage: inInit }">
      ...
      <view class="toolbar">
        ...
        <view
          v-if="props.isRotateBtn"
          @tap.stop="onAngle"
          class="btn-rotate"
        >
          <image src="/uni_modules/t-cropper/static/svg/rotate.svg" />
        </view>
...
const onAngle = () => {
  origin.rotate -= 90;
  var width = origin.bgImage.height;
  var height = origin.bgImage.width;
  var left = origin.bgImage.left;
  var top = origin.bgImage.top;
  var rate = width / height;
  if (width < origin.frame.width) {
    width = origin.frame.width;
    height = width / rate;
  }
  if (height < origin.frame.height) {
    height = origin.frame.height;
    width = height * rate;
  }
  if (left > origin.frame.left) {
    left = origin.frame.left;
  }
  if (top > origin.frame.top) {
    top = origin.frame.top;
  }
  if (left + width < origin.frame.left + origin.frame.width) {
    left = origin.frame.left + origin.frame.width - width;
  }
  if (top + height < origin.frame.top + origin.frame.height) {
    top = origin.frame.top + origin.frame.height - height;
  }
  origin.bgImage = {
    left: left,
    top: top,
...
const scaleFrame = (ta, tb) => {
  ...
  var x1 = move.start.frame.left;
  var y1 = move.start.frame.top;
  var x2 = move.start.frame.left + move.start.frame.width;
  var y2 = move.start.frame.top + move.start.frame.height;
  var cx1 = false;
  var cy1 = false;
  var cx2 = false;
  var cy2 = false;
...
const scaleImage = (ta, tb, tc, td) => {
  ...
  origin.bgImage.left =
    move.start.bgImage.left +
    ax -
    (ocx - move.start.bgImage.left) * (scale - 1);
  ...
  if (
    ...
    origin.bgImage.left =
      origin.frame.left + origin.frame.width - origin.bgImage.width;
  }
  if (
    ...
    origin.bgImage.top =
      origin.frame.top + origin.frame.height - origin.bgImage.height;
  }
};
